...
const mockDesktop = async (
  ...
  await page.addInitScript(({ updateStatus, desktopPlatform, permission }) => {
    ...
    Object.defineProperty(navigator.mediaDevices, "getDisplayMedia", {
      configurable: true,
      value: async () => {
        const captureStarts = Number(sessionStorage.getItem("capture-start-count") ?? "0") + 1;
        sessionStorage.setItem("capture-start-count", String(captureStarts));
...
test("carrega a tela inicial e abre a configuração do anfitrião", async ({ page }) => {
  ...
  const brandImages = page.locator(".brand-logo, .hero-device-card img");
  await expect(brandImages).toHaveCount(2);
  for (const image of await brandImages.all()) {
    await expect(image).toHaveAttribute("src", "./brand/infinity-app-icon.png");
...
test("copia o código da sala usando a ponte nativa", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: /Copiar código/i })).toBeVisible({ timeout: 3_000 });
  await expect(page.locator(".room-code-copy span")).toHaveText(code);
});

test("abre uma única sala mesmo com clique duplo e mantém o código estável", async ({ page }) => {
  await mockDesktop(page);
  await page.goto("/");
  await page.getByRole("button", { name: /Compartilhar minha tela/i }).click();
  await expect(page.locator(".source-card").first()).toBeVisible();
